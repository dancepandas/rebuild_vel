...
while true; do
  ATTEMPT=$((ATTEMPT + 1))
  echo "[mse_bc] training attempt $ATTEMPT at $(date '+%F %T')"
  "$PY" -m rebuild_vel.train --data data/vel --output "$RUN" --device cuda --seed 42 \
    ...
    --raw-hidden 64 --loss-mode mse_only --pure-attention --target-transform \
    >> "$RUN/train.log" 2>&1
  RC=$?
  if [ "$RC" -eq 0 ]; then
    echo "[mse_bc] training rc=0 at $(date '+%F %T')"
    break
  fi
  if grep -qE "CUDA out of memory|OutOfMemoryError" "$RUN/train.log"; then
    echo "[mse_bc] OOM; retrying in 10 min"
    sleep 600
  else
    echo "[mse_bc] TRAINING FAILED rc=$RC (not OOM)" | tee "$RUN/STATUS_TRAIN_FAILED.txt"
    exit "$RC"
  fi
done
...
"$PY" tools/plot_section_recon.py --checkpoint "$RUN/best.pt" \
  --outdir "$RUN/figs" --device cuda > /dev/null 2>&1
echo "[mse_bc] train + evals all done" | tee "$RUN/STATUS_DONE.txt"
